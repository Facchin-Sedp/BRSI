...
namespace BRSi
{
    public partial class ScheduledIFS : System.Web.UI.Page
    {

        public List<IFSdir> lista_tot;
        public List<IFSdir> lista_parziale = new List<IFSdir> { };
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                lista_tot = Read_Saved_IFS(" where TRIM(CFIFS) <>''");// leggo tutte le librerie salvate
                DDLlibrary.DataSource = lista_tot;
                DDLlibrary.DataTextField = "nome";
                DDLlibrary.DataValueField = "nome";
                DDLlibrary.DataBind();

                lblNumLib.Text = lista_tot.Count.ToString() + " IFS trovati";
                ViewState.Add("ListaTotale", lista_tot);
                ViewState.Add("ListaParziale", lista_parziale);
                BindRepeater(true);
            }
            else
            {
                lista_tot = (List<IFSdir>)ViewState["ListaTotale"];
                lista_parziale = (List<IFSdir>)ViewState["ListaParziale"];
            }
        ...
        protected void BtnNext_Click(object sender, EventArgs e)
        {
            ...
            lista_tot = Read_Saved_IFS(" where TRIM(CFIFS) <>''");// leggo tutte le librerie salvate
            ViewState["ListaTotale"] = lista_tot;
            BindRepeater(true);
        ...
        public List<IFSdir> Read_Saved_IFS(String Where)
        {
            List<IFSdir> rs = new List<IFSdir> { };
            ...
            if (rowset != null)
            {
                ...
                while (rowset.next())
                {
                    ...
                    l.nome = rowset.getString("CFIFS").Trim();
        ...
        {
            lista_tot = Read_Saved_IFS("where CFIFS like '" + Txtsearch.Text.ToUpper() + "%'");// leggo tutte le librerie salvate
        ...
        {
            lista_tot = Read_Saved_IFS("where trim(CFIFS) = '" + DDLlibrary.Text + "'");// leggo tutte le librerie salvate
            ViewState["ListaLibrerie"] = lista_tot;
